...
parser.add_argument('--description', default='KeAP20_bfs', type=str,
...
def train(model, graph, ppi_list, loss_fn, optimizer, device,
    ...
    for epoch in range(epochs):
        ...
        for step in range(steps):
            ...
            if got:
                output, _ = model(graph.x, graph.edge_index_got, train_edge_id)
                label = graph.edge_attr_got[train_edge_id]
            else:
                output, _ = model(graph.x, graph.edge_index, train_edge_id)
