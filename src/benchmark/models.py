...
class BertForOntoProteinContactPrediction(BertPreTrainedModel):
    ...
    def forward(self, input_ids, protein_length, attention_mask=None, labels=None, invalid_mask=None):
        ...
        loss_dmt = self.LossClass(inputs_embeds, inputs_embeds, sequence_output, sequence_output)
        loss = output_precition[0][0] + 0.001 * loss_dmt
        print('loss', output_precition[0][0].detach().cpu().numpy())
        print('loss_dmt', loss_dmt.detach().cpu().numpy())
        outputs['loss'] = loss
