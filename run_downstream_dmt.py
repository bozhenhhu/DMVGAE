...
class DynamicTrainingArguments(TrainingArguments):
    ...
    output_dir: str = field(
        default='contact-KeAP20-dmt',
        metadata={"help": "Directory path to save output files during training."}
    )
    do_train: bool = field(
    ...
    v_input: float = field(
        default=100)
    v_latent: float = field(
        default=0.01)
    sigmaP: float = field(
        default=1.0)
    augNearRate: float = field(
        default=100000)
    balance: float = field(
        default=1e-2)
...
class BTDataTrainingArguments:
    ...
    task_name: str = field(default='contact', metadata={"help": "The name of the task to train on: " + ", ".join(dataset_mapping.keys())})
    data_dir: str = field(
        default='downstream/',
